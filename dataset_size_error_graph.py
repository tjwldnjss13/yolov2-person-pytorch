...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_decay', type=float, required=False, default=.05)
    ...
    for e in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if (e + 1) % model_save_term == 0:
                save_pth = 'saved models/{}_{}_{}epoch_{}lr_{:.3f}loss(train)_{:.3f}loss_{:.3f}loss(coord)_{:.3f}loss(obj)_{:.3f}loss(noobj)_{:.3f}loss(cls).pth'.format(
                    model_name, dset_name, e + 1, cur_lr, train_loss_list[-1], val_loss_list[-1], val_loss_coord_list[-1],
